...
若`npm stop`关闭服务器失败，可以选择在命令行通过端口号来查找进程号PID来杀死进程从而关闭服务器。过程如下：

1. 在cmd中使用`netstat -ano`可以来查看所有PID进展号，如netstat命令不能被识别，则在环境变量path中添加路径，即可以找到此命令，添加`C:\windows\system32`和`C:\windows`
2. 使用`netstat -ano|findstr "7001"`来查找指定端口号的进程号，这里是找7001端口的进程。
3. 使用`tasklist|findstr "20232"`来查看进程号为20232的程序的相关信息
4. 使用`taskkill /F /pid "20232"`来杀死这个进程，从而停止服务器
